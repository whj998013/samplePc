    ...
    <Modal v-model="taskEditView" title="工序编辑" width="650px" class-name="vertical-center-modal">
      <Form :label-width="65" label-position="right">
        <row>
          ...
          <Col span="14">
          <FormItem label="姓名">
            <workerSelect v-model="currentTask.WorkerName" ref="wSelect" :HaveNoSelect="false" action="/ProofWorker/GetWorkerList/1"></workerSelect>
          </FormItem>
          </Col>
          <Col span="14">
          <FormItem label="计划日期">
          ...
          <FormItem label="任务开始方式">
            ...
            <!-- 任务选择 -->
            <Select v-if="!currentTask.IsBeginDate" style="width:200px">
...
export default {
  ...
  methods: {
    radioChange(v) {
      this.currentTask.IsBeginDate = v == "选择任务开始时间";
     // console.log(v);
    },
    EditTask(row) {
