  ...
  <div>
    ...
    <Tabs @on-click="tabClick" v-model="currentTab">
      <TabPane label="用纱申请" name="outApply">
        <OutStockApplyView v-if="tabPane.outApply" :dept="dept" ref="outApply" :action=$sra.yarn_GetYarnOutApplyList></OutStockApplyView>
      </TabPane>
      <TabPane label="入库记录" name="yarnIn">
        <YarnInStock v-if="tabPane.yarnIn" :dept="dept" ref="yarnIn" :action=$sra.yarn_GetMyYarnInStock></YarnInStock>
      </TabPane>
      <TabPane label="用纱记录" name="yarnOut">
        <YarnOutStock v-if="tabPane.yarnOut" :dept="dept" ref="yarnOut" :action=$sra.yarn_GetMyYarnOutStock></YarnOutStock>
      </TabPane>
