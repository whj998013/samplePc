  ...
  <div>
    ...
    <Modal v-model="showSumitBox" :title="'提交任务,工序：'+currentRow.ProcessName" width="650px">
      ...
      <Row>
        ...
        <Col span="8">
        <Upload type="drag" :with-credentials="true" :action="baseUrl + $sra.proof_UpLoadFile" :data="upLoadData" :on-success="fileUploadOUpSuccess" :show-upload-list=false>
          <div>
  ...
  methods: {
    async submit() {
      ...
      if (!haveError) {
        ...
        let re = await this.$util.post(this.$sra.proof_SubmitTask, this.currentTask);
    ...
    async getNextTask(TaskId) {
      let re = await this.$util.get(this.$sra.proof_GetNextTask + TaskId);
      console.log("re", re);
      if (re.data.length > 0) {
    ...
    async removeFile(f, index) {
      let fileid = f.Id;
      let re = await this.$util.get(this.$sra.proof_DeleteTaskFile + fileid);
      if ((re = "ok")) {
