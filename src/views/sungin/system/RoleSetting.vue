...
export default {
  ...
  methods: {
    syncWorker() {
      this.$util.get(this.$sra.setting_SyncWorker).then(p => {
        this.$Notice.success({
          title: "成功",
          desc: "同步工艺程序成功！",
          duration: 4
    ...
    upUserDataByDd() {
      ...
      this.$util.get(this.$sra.setting_UpUserDataByDd).then(p => {
        this.$Notice.success({
          title: "成功",
          desc: "同步用户角色成功！",
          duration: 4
    ...
    getData() {
      this.$bus.BeginLoading();
      this.$util.get(this.$sra.setting_GetUserRoleData).then(p => {
    ...
    savePermision() {
      ...
      this.$util.post(this.$sra.setting_SaveRoleData, obj).then(p => {
        this.getData();
      });
    }
  },
